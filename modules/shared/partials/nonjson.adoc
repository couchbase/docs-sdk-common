...
Additionally, non-JSON documents will not be accessible using the xref:{version-server}@server:manage:manage-ui/manage-ui.adoc[Web UI] (the contents will be shown in their Base64 equivalent).
// end::intro[]
...
Note that using non-JSON serialization will prevent the document from being accessible via {sqlpp}.
