...
NOTE: Although the API only sets expiry values _per document_, it is possible that elsewhere in the server, an expiry value is being set for xref:7.1@server:learn:buckets-memory-and-storage/expiration.adoc[every document in a bucket^].
