...
Transactions are distributed and work across multiple documents, scopes, and collections, which can reside on multiple nodes.
// end::intro[]
...
If at any point an error occurs, the transaction will rollback and the {lambda} will be run again.
// end::creating[]
...
The cleanup process is detailed in xref:concept-docs:transactions-error-cleanup.adoc#background-cleanup[Error Handling].
...
[NOTE]
.Couchbase Server
As of Couchbase Server 7.0, https://www.couchbase.com/products/n1ql[{sqlpp} (formerly N1QL)] queries may be used inside the transaction lambda, freely mixed with key-value operations.
