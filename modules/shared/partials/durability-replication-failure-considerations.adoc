...
All supported versions of Couchbase Server (since 6.5) offer xref:{version-server}@server:learn:data/durability.adoc[Durable Writes],
...
The three replication level options are:
...
In particular, `PersistToMajority` will take longer than the other two, and the `timeout` value needs to be selected with care -- particularly for large numbers of documents -- after testing on a representative network, with a realistic workload.
Variation in network speeds and conditions, _inter alia_, make it difficult to give blanket recommendations.

You can set Durablilty as part of regular xref:howtos:kv-operations.adoc#durability[CRUD operations against the Data Service],
or set it per Bucket from the Server or
xref:cloud:clusters:data-service/manage-buckets.adoc#configure-advanced-bucket-settings[Capella].
If it is set in either the SDK, the Server, or both, then the highest level on either side (or the level on the only side that sets it) is enforced.
// end::syncrep[]
...
-- see the diagram and explanation xref:{version-server}@server:learn:data/durability.adoc#process-and-communication[in the Server Durability docs].
...
Where the operation is an array append, or somesuch, then xref:{version-server}@server:learn:data/transactions.adoc[distributed, multi-document ACID transactions] should be considered.
...
Couchbase introduced xref:{version-server}@server:learn:data/transactions.adoc[distributed, multi-document ACID Transactions], with Couchbase Data Platform 6.5.
Read more on the concepts and commitment level, and take a look at the implementation:

* xref:server:learn:data/transactions.adoc[An introductory guide to how Couchbase ACID Transactions work].
