...
For maintainability, each adoc page in each SDK's docs repo to carry a form of the following:
...
// TIP - for those SDKs with all of the tutorials:
...
+
--
If you haven't already got a cluster set up, the easiest route is to https://cloud.couchbase.com/sign-up[sign up to a trial of Couchbase Capella^], 
...
+
--
You may want to look at one of the self-managed installation options:
...
[%collapsible]
====
[source,{lang-code},subs="+attributes"]
----
{complete-code-include}
----
====

Otherwise, read on as we introduce the CRUD API and connection to Capella or self-managed Couchbase Server.

TIP: There's a *View* link to the complete sample code on GitHub above each of the snippets on the page, and a *Copy* icon to grab just the snippet shown.
// end::complete-code[]


== Connect to your Database

Connect to your Couchbase Capella cluster (or your local Couchbase Cluster, if you are trying out self-managed Couchbase).
...
Each collection belongs to just one scope. 
...
* https://www.couchbase.com/forums/c/scala-sdk/37[Help forum].
* https://discord.com/channels/915294689681362954/955424070076338177[Discord channel].
...
* Explore some of the xref:project-docs:third-party-integrations.adoc[third party integrations] with Couchbase and the {name-sdk}, across the {name_platform} ecosystem.
// end::what-next[]
