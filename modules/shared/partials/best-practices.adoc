...
but given a sufficiently large operation the performance will tilt towards using a bulk get,
...
TIP: This bulk-get example is given for the {name-sdk} as there are times when for a sufficiently large operation it will be the most resource-efficient answer.
Nevertheless, Couchbase is a distributed database, and efficiently manages the fetching of your database from vbuckets spread across nodes --
in most cases just use the async API.
Should you wish to use a bulk get, be sure to benchmark performance in an environment which closely reflects your deployment environment, including network latency.
