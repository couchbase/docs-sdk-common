...
== Sub-Doc form the SDK

The Subdocument APIs is exposed through two separate builders which are available through two top-level Collection methods `LookupIn` and `MutateIn`. 
The object returned, and how to work with it, is extensively documented in our xref:howtos:subdocument-operations.adoc[practical uide to using Sub_Doc from the SDK].
