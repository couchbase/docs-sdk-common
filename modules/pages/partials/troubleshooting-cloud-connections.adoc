...
+
Check your IP is allowlisted in the https://docs.couchbase.com/cloud/index.html[Couchbase Capella] UI as an Allowed IP (make sure to hit Save after adding it).
...
All xref:howtos:managing-connections.adoc#ssl[recent versions] of the {name-sdk} include the Capella cluster's Security Certificate and automatically trust it unless you specify otherwise.
// All xref:{cert-link}[recent versions] of the {name-sdk} include the Capella cluster's Security Certificate and automatically trust it unless you specify otherwise.
* That if you are connecting via a proxy that intercepts TLS connections, you have configured the SDK to trust the Security Certificate used by the proxy.
This is a common issue when connecting from inside a corporate network.
