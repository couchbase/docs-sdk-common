...
Used in Concurrent Document Mutations for a type of optimistic locking.
...
* xref:howtos:health-check.adoc[Diagnostics] -- A diagnostics report just returns whatever current state the client is in.
This makes it much cheaper than `ping` to call on a regular basis, but does not provide any live insight into network slowness.
...
* xref:howtos:health-check.adoc[Health Check] -- Health Check provides `ping` and `diagnostics` tests for the health of the network and the cluster.
See under `ping` and `diagnostics` for more information.
...
* xref:concept-docs:response-time-observability.adoc[Observability] -- Various capabilities within the SDK provide more insight into their inner workings to help debug performance problems (such as latency spikes or reduced throughput) in production.
These cover Request Tracing, Observability Metrics, and Orphaned Response Reporting.
See the separate entries on this page for those three individual items.
* xref:howtos:observability-metrics.adoc[Observability Metrics] -- The SDK collects information which is not tied to an individual request but rather to a collection of them (such as requests per second on a specific socket), or concerning the environment the SDK is running in (including garbage collection timings andCPU usage, amongst others).
* xref:howtos:observability-orphan-logger.adoc[Orphaned Response Reporting] -- Special reporting capabilities which explicitly collect information about responses which have been abandoned (timed out) at the time when the SDK tries to complete them.* xref:howtos:n1ql-queries-with-sdk.adoc[Query] -- The Couchbase Query Service uses a SQL-like language for powerful queries across documents.
* xref:howtos:health-check.adoc[Ping] -- Ping provides information about the current state of the connections in the Couchbase cluster, by actively polling for the status of the specified services, giving status and latency information for every node reachable.
...
* xref:https://docs.couchbase.com/java-sdk/current/howtos/observability-tracing.adoc[Request Tracing] -- The tracing capability allows the SDK to store, aggregate, and potentially forward information about an individual request which can be used for later analysis.
Information such as the time it took to encode the request and how much time it spent on the wire.
