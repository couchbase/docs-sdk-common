...
[IMPORTANT]
====
This size limit is enforced by Couchbase Server (6.5 and earlier).
A compressed doument of just under 20MB, which is greater than 20MB in uncompressed form, will be accepted by the server;
however, if the Server needs to uncompress the document for a data operation or to send to an application with the compression mode set to `off`, an exception will be raised.
