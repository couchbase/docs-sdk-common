...
A new version of the Travel Sample application utilises xref:concept-docs:collections.adoc[Collections], introduced in the 7.0 release of the Couchbase Server.
...
IMPORTANT: The preceding documents are for example only and don't exist by default in the `travel-sample` bucket. 
