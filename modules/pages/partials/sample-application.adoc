...
If you already have Couchbase Server 4.5 or later installed but did not install the `travel-sample` bucket (or you installed it from a 4.0 version), open the Couchbase Web Console and select menu:Settings[Sample Buckets].
...
Go to menu:Indexes[Full Text, New Full Text Index] and create the `hotels` index:

image::{incimagesdir}/travel-app-fts-index.png[,650px]
...
. Sign in to Couchbase Travel by providing your credentials, or create a new account (which will expire after some time depending on the backend used, usually 1 hour):
+
...
image::{incimagesdir}/travel-app-hotel-data-model.png[]
