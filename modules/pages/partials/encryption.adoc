...
See the xref:java-sdk::encrypting-using-sdk.adoc[sample code page] for more secure code.
...
This is how Couchbase stores the JSON internally, but it is exposed to the developer as a public API (which currently has an https://developer.couchbase.com/documentation/server/5.5/sdk/encryption.html#story-h2-8[uncommited^] status).
For your preferred SDK, an API reference is available, linked from xref:java-sdk::start-using-sdk.adoc[Install and Start Using the Java SDK with Couchbase Server] as well as xref:java-sdk::encrypting-using-sdk.adoc[sample code].
