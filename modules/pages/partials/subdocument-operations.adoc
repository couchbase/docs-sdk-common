...
You have a choice of actually retrieving the document path using the _subdoc-get_ subdocument operation, or simply querying the existence of the path using the _subdoc-exists_ subdocument operation.
...
Even when modifying the _same_ part of the document, operations will not necessarily conflict, for example two concurrent _subdoc-array-append_ to the same array will both succeed, never overwriting the other.
